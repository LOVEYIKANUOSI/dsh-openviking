...
const OVCLI_PATH = () => path.join(homedir(), ".openviking", "ovcli.conf");
const DSH_CONFIG_PATH = () => path.join(homedir(), ".openviking", "dsh-config.json");
...
/**
 * 保存凭据到 ~/.openviking/ovcli.conf（合并现有字段，保留
 * auth_mode / actor_peer_id 等与本插件无关的键）。
 * 字段值为 undefined 表示不动；空串表示清除该字段。
 */
export function saveCredentials({ url, apiKey, account, user } = {}) {
  const filePath = OVCLI_PATH();
  const existing = readJsonFile(filePath);
  if (url !== undefined) {
    if (url === "") delete existing.url;
    else existing.url = String(url).replace(/\/+$/, "");
  }
  if (apiKey !== undefined) {
    if (apiKey === "") delete existing.api_key;
    else existing.api_key = String(apiKey);
  }
  if (account !== undefined) {
    if (account === "") delete existing.account;
    else existing.account = String(account);
  }
  if (user !== undefined) {
    if (user === "") delete existing.user;
    else existing.user = String(user);
  }
...
/**
 * 保存行为配置到 ~/.openviking/dsh-config.json（浅合并，保留未知字段）。
 * 注意：若用户设置了 OPENVIKING_PLUGIN_CONFIG 或
 * $DSH_HOME/openviking-config.json，它们的优先级高于此文件，
 * 保存后未必生效——UI 通过 configPath 展示实际生效来源。
 */
export function saveBehaviorConfig(section) {
  const filePath = DSH_CONFIG_PATH();
  const existing = readJsonFile(filePath);
  writeJsonFile(filePath, { ...existing, ...(section ?? {}) });
  return filePath;
}

/**
 * 热重载：重新解析完整配置并把结果原地合并进 shared 对象。
 * 所有闭包持有 shared 的顶层引用，因此凭据与行为改动即时生效，
 * 无需重启 dsh。MCP proxy 子进程通过自身的凭据文件监听独立热重载。
 */
export function reloadSharedConfig(shared, pluginRoot, entryConfig = {}) {
  const next = loadConfig(pluginRoot, entryConfig);
  Object.assign(shared, next);
  return shared;
}

/** 行为字段白名单（HTTP 设置页可写字段）。 */
