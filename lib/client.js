	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			ctx.slots.inject("settings.plugins.tab", () => ctx.slots.register({
				...
				label: () => t("tab"),
				locale: NS,
				// 组件定义在模块作用域（factory 顶层），拿不到 apply 的局部
				// 变量，翻译函数必须经 slot inject 以 props 传入。
				inject: () => ({ t }),
			}, (props) => React.createElement(OpenVikingSettingsTabBoundary, null, React.createElement(OpenVikingSettingsTab, { t: props?.t }))));
		}

		/** 自诊断错误边界：渲染异常时把错误显示出来而不是留白。 */
		class OpenVikingSettingsTabBoundary extends React.Component {
			constructor(props) {
				super(props);
			...
			render() {
				if (this.state.error) {
					return React.createElement("div", { className: "ov-wrap" },
						React.createElement("p", { className: "ov-status", "data-kind": "error" }, "OpenViking tab render failed"),
						React.createElement("pre", { style: { whiteSpace: "pre-wrap", fontSize: 12 } }, String(this.state.error && (this.state.error.stack || this.state.error))),
					);
				}
		...
		function OpenVikingSettingsTab(props) {
			...
			const load = React.useCallback(() => {
				fetch("/api/openviking/config", { headers: { "x-dsh-openviking": "dsh-openviking" } })
					.then((r) => r.json())
					.then((data) => {
			...
			const save = () => {
				...
				fetch("/api/openviking/config", {
					method: "POST",
					headers: { "Content-Type": "application/json", "x-dsh-openviking": "dsh-openviking" },
